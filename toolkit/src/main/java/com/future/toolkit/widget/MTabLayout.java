...
import com.future.toolkit.utils.AnimationUtils;
import com.future.toolkit.utils.AppCompatUtils;
...
public class MTabLayout extends HorizontalScrollView {
    ...
    private static final int DEFAULT_TAB_SIZE = 24; //dps
    ...
    public MTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        AppCompatUtils.checkAppCompatTheme(context);
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MTabLayout,
                defStyleAttr, R.style.MTabLayoutStyle);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabIndicatorHeight, 0));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.MTabLayout_mt_tabIndicatorColor, 0));
        mTabIndicatorPadding = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabIndicatorPadding, 0);
        mTabIndicatorWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabIndicatorWidth, 0);


        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.MTabLayout_mt_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.MTabLayout_mt_tabTextAppearance,
                R.style.MTabTextAppearance);
        ...
        if (a.hasValue(R.styleable.MTabLayout_mt_tabTextSize)){
            mTabTextSize = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabTextSize, 0);
        }

        if (a.hasValue(R.styleable.MTabLayout_mt_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.MTabLayout_mt_tabTextColor);
        }

        if (a.hasValue(R.styleable.MTabLayout_mt_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.MTabLayout_mt_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.MTabLayout_mt_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabContentStart, 0);
        mMode = a.getInt(R.styleable.MTabLayout_mt_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.MTabLayout_mt_tabGravity, GRAVITY_FILL);

        final Resources res = getResources();
        mTabTextMultiLineSize = a.getDimension(R.styleable.MTabLayout_mt_tabTextMultiLineSize,
                mTabTextMultiLineSize);
        mScrollableTabMinWidth = a.getDimensionPixelSize(R.styleable.MTabLayout_mt_tabScrollableTabMinWidth, mScrollableTabMinWidth);

        mTabMinHeight = a.getDimensionPixelOffset(R.styleable.MTabLayout_mt_tabMinHeight, dpToPx(DEFAULT_HEIGHT));
     ...
     *
     * @attr ref android.support.design.R.styleable#MTabLayout_tabIndicatorColor
     */
     ...
     *
     * @attr ref android.support.design.R.styleable#MTabLayout_tabIndicatorHeight
     */
     ...
     *
     * @attr ref android.support.design.R.styleable#MTabLayout_tabMode
     */
     ...
     *
     * @attr ref android.support.design.R.styleable#MTabLayout_tabGravity
     */
     ...
     * Sets the text colors for the different states (normal, selected) used for the tabs.
     *
     * @attr ref android.support.design.R.styleable#MTabLayout_tabTextColor
     * @attr ref android.support.design.R.styleable#MTabLayout_tabSelectedTextColor
     */
     ...
     * The one-stop shop for setting up this {@link MTabLayout} with a {@link ViewPager}.
     *
     * <p>This method will link the given ViewPager and this MTabLayout together so that
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mIconView == null) {
                    ...
                    int size = dpToPx(DEFAULT_TAB_SIZE);
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            ...
            if (mIndicatorLeft >= 0 && mIndicatorRight > mIndicatorLeft) {
                int padding = mTabIndicatorPadding;
                if (mTabIndicatorWidth > 0 && mIndicatorRight - mIndicatorLeft > mTabIndicatorWidth){
                    padding = (mIndicatorRight - mIndicatorLeft - mTabIndicatorWidth) / 2;
                }
                int radius = mSelectedIndicatorHeight / 2;
                mTempRect.set(mIndicatorLeft + padding, getHeight() - mSelectedIndicatorHeight,
                        mIndicatorRight - padding, getHeight());
                canvas.drawRoundRect(mTempRect , radius, radius, mSelectedIndicatorPaint);
            }
        }
    }
    ...
    private int getDefaultHeight() {
        ...
        return hasIconAndText ?dpToPx(DEFAULT_HEIGHT_WITH_TEXT_ICON) : mTabMinHeight;
    }
